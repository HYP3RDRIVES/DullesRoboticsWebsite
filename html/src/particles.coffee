...
pJS = (tag_id, params) ->
  ...
  pJS.fn.retinaInit = ->
    if pJS.retina_detect and window.devicePixelRatio > 1
  ...
  pJS.fn.canvasSize = ->
    ...
    if pJS and pJS.interactivity.events.resize
  ...
  pJS.fn.particle = (color, opacity, position) ->
    ...
    @radius = (if pJS.particles.size.random then Math.random() else 1) * pJS.particles.size.value
    if pJS.particles.size.anim.enable
      @size_status = false
      @vs = pJS.particles.size.anim.speed / 100
      if !pJS.particles.size.anim.sync
        @vs = @vs * Math.random()

    ### position ###

    @x = if position then position.x else Math.random() * pJS.canvas.w
    @y = if position then position.y else Math.random() * pJS.canvas.h

    ### check position  - into the canvas ###

    if @x > pJS.canvas.w - (@radius * 2)
      @x = @x - (@radius)
    else if @x < @radius * 2
      @x = @x + @radius
    if @y > pJS.canvas.h - (@radius * 2)
      @y = @y - (@radius)
    else if @y < @radius * 2
      @y = @y + @radius
    ...
    if typeof color.value == 'object'
      if color.value instanceof Array
        color_selected = color.value[Math.floor(Math.random() * pJS.particles.color.value.length)]
        @color.rgb = hexToRgb(color_selected)
      else
        if color.value.r != undefined and color.value.g != undefined and color.value.b != undefined
          @color.rgb =
            r: color.value.r
            g: color.value.g
            b: color.value.b
        if color.value.h != undefined and color.value.s != undefined and color.value.l != undefined
          @color.hsl =
            h: color.value.h
            s: color.value.s
            l: color.value.l
      ...
      @color = color
      @color.rgb = hexToRgb(@color.value)

    ### opacity ###

    @opacity = (if pJS.particles.opacity.random then Math.random() else 1) * pJS.particles.opacity.value
    if pJS.particles.opacity.anim.enable
      @opacity_status = false
      @vo = pJS.particles.opacity.anim.speed / 100
    ...
    shape_type = pJS.particles.shape.type
    ...
    if @shape == 'image'
      ...
      if !@img.ratio
        @img.ratio = 1
      if pJS.tmp.img_type == 'svg' and pJS.tmp.source_svg != undefined
        pJS.fn.vendors.createSvgImg this
        if pJS.tmp.pushing
          @img.loaded = false
    return

  pJS.fn.particle::draw = ->
    `var radius`
    `var opacity`
    `var color_value`
    `var img_obj`
  ...
  pJS.fn.particlesCreate = ->
    i = 0
    while i < pJS.particles.number.value
  ...
  pJS.fn.particlesUpdate = ->
    `var new_pos`
    i = 0
    while i < pJS.particles.array.length
      ...
      if isInArray('bubble', pJS.interactivity.events.onhover.mode) or isInArray('bubble', pJS.interactivity.events.onclick.mode)
        pJS.fn.modes.bubbleParticle p
      if isInArray('repulse', pJS.interactivity.events.onhover.mode) or isInArray('repulse', pJS.interactivity.events.onclick.mode)
        pJS.fn.modes.repulseParticle p

      ### interaction auto between particles ###

      if pJS.particles.line_linked.enable or pJS.particles.move.attract.enable
        j = i + 1
        while j < pJS.particles.array.length
          p2 = pJS.particles.array[j]
  ...
  pJS.fn.interact.linkParticles = (p1, p2) ->
    ...
    if dist <= pJS.particles.line_linked.distance
      opacity_line = pJS.particles.line_linked.opacity - (dist / (1 / pJS.particles.line_linked.opacity) / pJS.particles.line_linked.distance)
      if opacity_line > 0
  ...
  pJS.fn.interact.attractParticles = (p1, p2) ->
    ...
    dist = Math.sqrt(dx * dx + dy * dy)
    if dist <= pJS.particles.line_linked.distance
      ax = dx / (pJS.particles.move.attract.rotateX * 1000)
  ...
  pJS.fn.modes.pushParticles = (nb, pos) ->
    ...
    while i < nb
      pJS.particles.array.push new (pJS.fn.particle)(pJS.particles.color, pJS.particles.opacity.value,
        'x': if pos then pos.pos_x else Math.random() * pJS.canvas.w
        'y': if pos then pos.pos_y else Math.random() * pJS.canvas.h)
      if i == nb - 1
        if !pJS.particles.move.enable
          pJS.fn.particlesDraw()
  ...
  pJS.fn.modes.bubbleParticle = (p) ->
    ...
    process = (bubble_param, particles_param, p_obj_bubble, p_obj, id) ->
      `var obj`
      if bubble_param != particles_param
        if !pJS.tmp.bubble_duration_end
          ...
          if p_obj_bubble != undefined
            value_tmp = p_obj - (time_spent * (p_obj - bubble_param) / pJS.interactivity.modes.bubble.duration)
    ...
    if pJS.interactivity.events.onhover.enable and isInArray('bubble', pJS.interactivity.events.onhover.mode)
      ...
      if dist_mouse <= pJS.interactivity.modes.bubble.distance
        if ratio >= 0 and pJS.interactivity.status == 'mousemove'
          ...
          if pJS.interactivity.modes.bubble.opacity != pJS.particles.opacity.value
            if pJS.interactivity.modes.bubble.opacity > pJS.particles.opacity.value
              ...
              if opacity > p.opacity and opacity <= pJS.interactivity.modes.bubble.opacity
      ...
      if pJS.interactivity.status == 'mouseleave'
        init()
    else if pJS.interactivity.events.onclick.enable and isInArray('bubble', pJS.interactivity.events.onclick.mode)
  ...
  pJS.fn.modes.repulseParticle = (p) ->
    ...
    if pJS.interactivity.events.onhover.enable and isInArray('repulse', pJS.interactivity.events.onhover.mode) and pJS.interactivity.status == 'mousemove'
      dx_mouse = p.x - (pJS.interactivity.mouse.pos_x)
      dy_mouse = p.y - (pJS.interactivity.mouse.pos_y)
      dist_mouse = Math.sqrt(dx_mouse * dx_mouse + dy_mouse * dy_mouse)
      normVec = 
      ...
      if pJS.particles.move.out_mode == 'bounce'
        if pos.x - (p.radius) > 0 and pos.x + p.radius < pJS.canvas.w
          p.x = pos.x
        if pos.y - (p.radius) > 0 and pos.y + p.radius < pJS.canvas.h
          p.y = pos.y
      else
        p.x = pos.x
        p.y = pos.y
    else if pJS.interactivity.events.onclick.enable and isInArray('repulse', pJS.interactivity.events.onclick.mode)
      if !pJS.tmp.repulse_finish
        pJS.tmp.repulse_count++
        if pJS.tmp.repulse_count == pJS.particles.array.length
          pJS.tmp.repulse_finish = true
      if pJS.tmp.repulse_clicking
        repulseRadius = (pJS.interactivity.modes.repulse.distance / 6) ** 3
        dx = pJS.interactivity.mouse.click_pos_x - (p.x)
        dy = pJS.interactivity.mouse.click_pos_y - (p.y)
        d = dx * dx + dy * dy
        force = -repulseRadius / d * 1
        # default
        if d <= repulseRadius
          process()
  ...
  pJS.fn.modes.grabParticle = (p) ->
    if pJS.interactivity.events.onhover.enable and pJS.interactivity.status == 'mousemove'
      dx_mouse = p.x - (pJS.interactivity.mouse.pos_x)
      dy_mouse = p.y - (pJS.interactivity.mouse.pos_y)
      dist_mouse = Math.sqrt(dx_mouse * dx_mouse + dy_mouse * dy_mouse)
  ...
  pJS.fn.vendors.eventsListeners = ->
    ...
    if pJS.interactivity.events.onhover.enable or pJS.interactivity.events.onclick.enable
      ...
      pJS.interactivity.el.addEventListener 'mousemove', (e) ->
        ...
        if pJS.interactivity.el == window
          ...
          pos_x = e.offsetX or e.clientX
          pos_y = e.offsetY or e.clientY
  ...
  pJS.fn.vendors.densityAutoParticles = ->
    if pJS.particles.number.density.enable
      ...
      area = pJS.canvas.el.width * pJS.canvas.el.height / 1000
      if pJS.tmp.retina
  ...
  pJS.fn.vendors.checkOverlap = (p1, position) ->
    i = 0
    while i < pJS.particles.array.length
      p2 = pJS.particles.array[i]
      dx = p1.x - (p2.x)
      dy = p1.y - (p2.y)
      dist = Math.sqrt(dx * dx + dy * dy)
      if dist <= p1.radius + p2.radius
        p1.x = if position then position.x else Math.random() * pJS.canvas.w
        p1.y = if position then position.y else Math.random() * pJS.canvas.h
...
window.particlesJS.load = (tag_id, path_config_json, callback) ->
  ...
  xhr.onreadystatechange = (data) ->
    if xhr.readyState == 4
      if xhr.status == 200
        params = JSON.parse(data.currentTarget.response)
        window.particlesJS tag_id, params
        if callback
          callback()
      else
